...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		// 刷新分类
		function refresh() {
			return service.project.doccategory.list().then((res: any) => {
				res.unshift({
					name: "全部文件",
					id: null
				});
			...
			refs.value.form.open({
				...
				on: {
					submit: (data: any, { done, close }: any) => {
						...
						if (!item.id) {
							next = service.project.doccategory.add(data);
						} else {
							next = service.project.doccategory.update({
								...data,
								id: item.id
							});
						}

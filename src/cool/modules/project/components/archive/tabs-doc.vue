<template>
	<div class="archive-doctdetail scroller1">
		<div class="pane">
			<!-- 文档架构 -->
			<div class="archive" :class="[isExpand ? '_expand' : '_collapse']">
				<archive-tree
					ref="archive_treeRef"
			...
			<div class="doc">
				...
				<div class="container">
					<tree-context @treeRefresh="reqRefresh" :archive="true"></tree-context>
				</div>
			</div>
		</div>
	</div>
...
export default defineComponent({
    ...
    provide('doc', docObj);
		...
		const reqRefresh = async () => {
			const selected = await archive_treeRef.value.getCurrentNode();
		  await	archive_treeRef.value.refresh();
			const nowSelect = await archive_treeRef.value.getNode(selected);
		...
		function onNodeRowClick({ item, ids }: any) {
			selectNode.value = {
				...item,
		  	root: archive_treeRef.value.list,
			} 
